function initCommonClsGiant(moduleId, layout) {
	...
	if($.inArray(layout,['101','102','103','104','105','108','109']) > -1){
		...
		$("#module_" + moduleId).find('.main-class-item a,.sub-class-item a').each(function () {
		...
		//if(curclass == null) curclass = $("#module_" + moduleId + " .main-class-item:first");
    ...
    if ($.inArray(layout,['101','102']) > -1) {
        ...
        var windowchange = function () {
            ...
            if (windowwidth <= 425) {
                //小屏
                //$('#module_' + moduleId + ' .classify').next().hide();
        ...
        function windowchange103() {
            ...
            var windowchange = function () {
                var windowwidth = $(window).width();
            if (windowwidth < 761 || (layout == '110' && windowwidth < 1200) || (layout == '109' && windowwidth < 975)|| (layout == '108' && windowwidth < 975) ) {
                    var documentscrootop = 0;
                    //生成遮罩层和弹窗内容放入body
                    //$('.layout-104dialog').not($('.layout-104dialog' + moduleId)).remove();
                    $('.mask-layermask-layer').remove();
                    if ($('.layout-' + layout + 'dialog'+moduleId).length <= 0) { /* style="padding-top:50px"要判断是否有头部*/
                        $('body').append('<div class="ModuleCommonClsGiant  layout-' + layout + ' layout-' + layout + 'dialog layout-' + layout + 'dialog' + moduleId + '"><div class="layout' + layout + 'dialog-left"></div><div class="inlayout-' + layout + 'dialog'+moduleId+'" style="position:absolute;width:79%;top:0;left:21%;box-sizing:border-box;">' + $('#module_'+moduleId+' .main-class-container-mobile').html() + '</div></div>');
                        $('body').append('<div class="mask-layermask-layer mask-layermask-layer' + moduleId + '" style="height:100%;padding-bottom:55px;width:100%;position: fixed;top:0;left:0;display: none;background:rgba(0,0,0,.5)"></div>');
                    }else{
                    ...
                    if($('.mask-layermask-layer'+moduleId).length<=0){
                        $('body').append('<div class="mask-layermask-layer mask-layermask-layer' + moduleId + '" style="height:100%;width:100%;position: fixed;top:0;left:0;display: none;background:rgba(0,0,0,.5)"></div>');
                    }
                    ...
                    if ($.inArray(layout,['109']) > -1) {
                        $('.layout-' + layout + 'dialog' + moduleId + ' .sub-class-item').off('click').on('click touch', function () {
                            ...
                            //_this.find('.main-class-icon').toggleClass('icon-xiangshangjiantou');
...
function initCommonClsPc106(moduleId) {
    ...
    $(mainItemsrow).off('mouseenter').on('mouseenter',function(){
        ...
        //console.log($(cloneId).find('.sub-class-items'),$(cloneId).find('.sub-class-items').length())
        ...
        var xianWidth = Number($(this).css('border-right-width').replace('px',''))+1;
        oxian.css({
            'height':$(this).height(),
            'width':xianWidth,
            'position':'absolute',
            'left': ($(this).closest('ul').offset().left + $(this).outerWidth()-1) + 'px',
            'top':$(this).offset().top+Number($(this).css('border-top-width').replace('px','')), 
            'background':'#fff',
            'z-index':20,
        })
...
function initCommonClsMob106(moduleId) {
    ...
    // 鼠标滚动
    // $(mobileBox).find(".main-class-list-content").mCustomScrollbar({
    //     scrollInertia: 200
    // });
    // 给需要滚动的盒子的父盒子添加高度
    parentAddHeight();
    ...
    document.querySelector(cloneIdMob + " .main-class-container-mobile .main-class-content-title .close").onclick = function () {
        ...
        leftLucency.hide();
        // if(mobileBox.style.transform=='translateX(100%)'){
    ...
    showbox.onclick = function () {
        ...
        /* if($('#MobileNav').is(':hidden')){
            mobileBox.style.top = 0;
        }else{
            console.log($('#MobileNav').position)
            if($('#MobileNav').position.bottom!=0){
                mobileBox.style.top = 0;
            }else{
                mobileBox.style.top = $('#MobileNav').outerHeight()+'px';
            }
...
function resetHeightThreeimg(moduleId,layout){
    ...
    // if(layout == 107){
    //     cloneBox = "#clone_mob_module_" + moduleId;
    // };
